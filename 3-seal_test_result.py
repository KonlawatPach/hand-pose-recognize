...
# Load Model & Scaler
with open('model-seal.pkl', 'rb') as file:
    model = pickle.load(file)
with open('scalar.pkl', 'rb') as file:
    scaler = pickle.load(file)

# ล้างมือกี่วินาที
timeflame_success_perhand = 20
...
def add_hand_success(class_num):
    global hand_success
    class_num = seal_to_int_map[class_num]
...
def check_hand_success(frame, width, height):
    ...
    if(count_hand_success < class_count):
        frame = put_text_pil(frame, "ประสานอินไม่ครบ\nไปประสานอินใหม่นะ", 
